...
| メリット | 仕組み |
|---|---|
| **メインモデルの精度 × サブモデルの速度・低コスト** | 高精度なメインモデル（Claude Opus, GPT Codex 等）は判断だけを行い、実作業は安価なサブモデルが担当します。メインモデルの賢さを保ちつつ、トークンコストは大幅に抑えられます。 |
...
- **ロール別オーバーライド**（例: `planner` → 大窓モデル）は任意ですが、特定のロールがモデルの
  特性を有利に活かせる場合は推奨します。
