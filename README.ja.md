...
| `/implementer` | プランを実装する | **中級メインが自ら実装** ＋ `Researcher`（調べもの） ＋ `verifier`（最終 DoD 判定） ＋ `reviewer`（diff 品質レビュー） |
| `/tester` | プランの Definition-of-Done を再検証する | `verifier` |
| `/reviewer` | diff の correctness/design/security/maintainability をレビューする | `reviewer` |
...
セッション 2（クリーンなコンテキスト）
  ...
  /implementer <プランファイル>
    ...
    → PASS なら reviewer サブエージェントで diff 品質レビュー → Blocker/Major を修正
    → 完了（任意: 後で /tester による独立 DoD 再検証）
```
...
3. `~/.pi/agent/extensions/subagent-models.json` を編集し、プレースホルダのモデルID
   （`YOUR_LOCAL_MODEL`、`YOUR_REMOTE_MODEL`、`YOUR_CLOUD_MODEL`、`YOUR_REASONING_MODEL`、
   `YOUR_REVIEWER_MODEL`）を、あなたの Pi `models.json` や `models.generated.js` に存在する
   モデルIDへ置き換えます。
...
```
agents/            # サブエージェント定義 — ロールフローで実際に spawn されるのは Researcher、verifier、reviewer。
...
extensions/
  ...
  reviewer-orchestrator.md    # /reviewer プロンプト
