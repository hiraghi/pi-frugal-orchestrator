...
| メリット | 仕組み |
|---|---|
| **メインモデルの精度 × サブモデルの速度・低コスト** | 高精度なメインモデル（Claude Opus, GPT-4.1 等）は判断だけを行い、実作業は安価なサブモデルが担当。メインモデルの賢さを保ちつつ、トークンコストは大幅に抑えられる。 |
...
**モデル選定の指針:**
- **ローカルLLMがある場合**: `defaults[0]` にローカルモデル（例: llama.cpp 経由の Qwen-27B）を指定。
...
│  │ FAIL → implementer が修正 → 再検証（PASS までループ）          │    │
│  └─────────────────────────────────────────────────────────────┘    │
│  完了 → /tester で独立した最終検証                                     │
└─────────────────────────────────────────────────────────────────────┘
```
